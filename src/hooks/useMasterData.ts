...
export function useMasterData(category?: string) {
  ...
  const createItem = async (itemData: Partial<MasterData>) => {
    try {
      ...
      const err = error as ApiErrorResponse;
      toast.error(err.response?.data?.message || 'Failed to create master data');
      return null;
    }
  };

  const updateItem = async (id: string, itemData: Partial<MasterData>) => {
    try {
      ...
      const err = error as ApiErrorResponse;
      toast.error(err.response?.data?.message || 'Failed to update master data');
      return null;
    }
  };
